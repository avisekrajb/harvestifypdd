...
# MongoDB Connection
MONGODB_URI = os.getenv('MONGODB_URI', 'mongodb+srv://rajbanshi:hardware@cluster0.so5agoe.mongodb.net/?appName=Cluster0')
MONGODB_DB = os.getenv('MONGODB_DB', 'harvestify')
...
def seed_admin():
    ...
    try:
        ...
        existing_admin = db.users.find_one({'email': 'admin@harvestify.com'})
        
        if existing_admin:
            print("✓ Admin user already exists!")
            print(f"  Email: admin@harvestify.com")
        ...
        admin_data = {
            ...
            'phone': '9876543210',
            'address': 'Harvestify Admin Office',
            'role': 'admin',
        ...
        verify = db.users.find_one({'email': 'admin@harvestify.com'})
        if verify:
...
def seed_products():
    ...
    try:
        ...
        if product_count > 0:
            print(f"✓ Products already exist ({product_count} products)")
            return
...
def main():
    ...
    print("\n" + "=" * 50)
    print("🌾 HARVESTIFY DATABASE SEEDER")
    print("=" * 50 + "\n")
    ...
    print("   Email:    admin@harvestify.com")
